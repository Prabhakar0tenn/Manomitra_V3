from utils.db import db_manager
from utils.gemini_client import generate_notes as gemini_generate_notes
from fastapi import HTTPException
from datetime import datetime

def generate_study_notes(document_id: str, detail_level: str) -> dict:
    collection = db_manager.get_collection("study_hub_documents")
    doc = collection.find_one({"document_id": document_id})
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")
        
    context = doc.get("extracted_text", "")
    if not context or not context.strip():
        raise HTTPException(
            status_code=400, 
            detail="Could not extract text from PDF. Make sure it is not a scanned image-only PDF"
        )
        
    doc_name = doc.get("original_name", "Document")
    
    result = gemini_generate_notes(context, doc_name, detail_level)
    result["generated_at"] = datetime.utcnow()
    
    return result
